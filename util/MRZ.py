...
class MRZ(object):
    ...
    def __init__(self, mrz_lines, mrz_type=None):
        """
    ...
    def __repr__(self):
        if self.valid:
            return "MRZ({0}[valid], {1}, {2}, {3}, {4}, {5}, {6})".format(self.mrz_type, self.number, self.names,
                                                                     self.surname, self.sex, self.date_of_birth, self.expiration_date)
        elif self.valid_score > 0:
            return "MRZ({0}[{1}], {2}, {3}, {4}, {5}, {6}, {7})".format(self.mrz_type, self.valid_score, self.number,
                                                                   self.names, self.surname, self.sex,
                                                                   self.date_of_birth, self.expiration_date)
        else:
            return "MRZ(invalid)"
    ...
    def _guess_type(mrz_lines):
        ...
        """
        try:
            if len(mrz_lines) == 1 and mrz_lines[0][0:2].upper() == 'CS':
                return 'CHNHK2014'
            elif len(mrz_lines) == 1 and mrz_lines[0][0:2].upper() == 'CD':
                return 'CHNTW2014'
                ...
                if mrz_lines[0][0].upper() == 'V':
                    return 'MRVA'
                elif mrz_lines[0][0:2].upper() == 'W<':
                    return 'CHNHK2000'
                elif mrz_lines[0][0:2].upper() == 'D<':
                    return 'CHNTW2000'
                elif mrz_lines[0][0:2].upper() == 'T<':
                    return 'TWCHN2000'
                else:
                    return 'TD3'
            else:
    ...
    def _parse(self, mrz_lines):
        ...
        try:
            ...
            if self.mrz_type == 'TD1':
                self.mrz_line1 = mrz_lines[0]
                self.mrz_line2 = mrz_lines[1]
                self.mrz_line3 = mrz_lines[2]
            ...
            elif self.mrz_type in ('CHNHK2000', 'CHNTW2000', 'TWCHN2000'):
                self.mrz_line1 = mrz_lines[0]
                self.mrz_line2 = mrz_lines[1]
                self.valid = self._parse_china_hk_tw_pass_v2000(*mrz_lines)
            elif self.mrz_type in ('CHNHK2014', 'CHNTW2014'):
                self.mrz_line1 = mrz_lines[0]
                self.valid = self._parse_china_hk_tw_pass_v2014(*mrz_lines)
    ...
    def to_dict(self):
        ...
        if self.mrz_type is not None:
            ...
            result['mrz_line1'] = self.mrz_line1
            result['mrz_line2'] = self.mrz_line2
            result['mrz_line3'] = self.mrz_line3
        return result
    ...
    def _parse_td1(self, a, b, c):
        try:
            ...
            self.valid_check_digits = [MRZCheckDigit.compute(self.number) == self.check_number,
                                       MRZCheckDigit.compute(
                                           self.date_of_birth) == self.check_date_of_birth and MRZ._check_date(
                                           self.date_of_birth),
                                       MRZCheckDigit.compute(
                                           self.expiration_date) == self.check_expiration_date and MRZ._check_date(
            ...
            logging.error('Error when running function %s, error = %s' % (sys._getframe().f_code.co_name, str(e)))
        finally:
    ...
    def _parse_td3(self, a, b):
        try:
            ...
            self.type = a[0:2]
            self.country = a[2:5]
            if self.country == 'MYS' or b[10:13] == 'MYS':
    ...
    # 中华人民共和国往来港澳通行证 v2000
    # 大陆居民往来台湾通行证 v2000
    # 台湾居民往来大陆通行证 v2000
    def _parse_china_hk_tw_pass_v2000(self, a, b):
        try:
            len_a, len_b = len(a), len(b)
            if len(a) < 44:
                a = a + '<' * (44 - len(a))
            ...
            # 机读码第一行，共有44位字符
            # 第1~2位，证件名称代码，用"W<"表示2000版往来港澳通行证
            self.type = a[0:2]

            # 第3~5位，签发国代码，用大写拉丁字母“CHN”表示中国
            self.country = a[2:5]

            # 第6~41位，持证人中文姓名，姓在前，名在后；共表示9个汉字，姓名大于9个汉字是取前9个，其余汉字自动舍弃，小于9个时尾部用"<"补齐
            surname_names = a[5:41].split('<<', 1)
            if len(surname_names) < 2:
                surname_names += ['']
            self.surname, self.names = surname_names
            self.names = self._gbk_conv(self.surname)
            self.surname = ''

            # 机读码第二行，共有44位字符
            # 第1~9位，证件号码，其中第1位用大写拉丁字母“W”表示；第2~9位为顺序号，用8位阿拉伯数字表示
            self.number = b[0:9]

            # 第10位，1~9位的检验位
            self.check_number = b[9]

            # 第11~13位，签发国代码，用大写拉丁字母“CHN”表示中国
            self.nationality = b[10:13]

            # 第14~19位，持证人出生年月日，YYMMDD
            self.date_of_birth = b[28:30] + b[13:19]
            ...
            # 第22~27位，证件有效期截止年月日，YYMMDD
            self.expiration_date = b[30:32] + b[21:27]

            # 第28位，22~27位的检验位
            self.check_expiration_date = b[27]

            # 第29~30位，持证人出生年的前2位
            # 第31~32位，证件有效期截止年的前2位
            # 第33~36位，签发机关代码，应符合 GA/T 704.19-2007 的规定
            # 第37~42位，空位，
            self.personal_number = b[28:42]
            ...
            self.valid_check_digits = [MRZCheckDigit.compute(self.number) == self.check_number,
                                       ...
                                       MRZCheckDigit.compute(
                                           self.expiration_date) == self.check_expiration_date and MRZ._check_date(
                                           self.expiration_date),
                                       MRZCheckDigit.compute(self.personal_number) == self.check_personal_number,
                                       MRZCheckDigit.compute(b[0:10] + b[13:20] + b[21:43]) == self.check_composite]
            self.valid_line_lengths = [len_a == 44, len_b == 44]
            self.valid_misc = [a[0] in ('W','D','T')]
            self.valid_score = 10 * sum(self.valid_check_digits) + sum(self.valid_line_lengths) + sum(self.valid_misc) + 1
            self.valid_score = 100 * self.valid_score // (50 + 2 + 1 + 1)
            self.valid_number, self.valid_date_of_birth, self.valid_expiration_date, self.valid_personal_number, self.valid_composite = self.valid_check_digits
        except Exception as e:
    ...
    # 往来港澳通行证 v2014
    # 往来台湾通行证 v2014
    def _parse_china_hk_tw_pass_v2014(self, a):
        try:
            len_a = len(a)
            if len(a) < 30:
                a = a + '<' * (30 - len(a))

            # Set default value as they are not available from the MRZ
            self.names = ''
            self.surname = ''
            self.country = 'CHN'
            self.nationality = 'CHN'
            self.sex = ''

            # 机读码第一行，共有30位字符
            # 第1~2位，证件名称及种类代码，用大写拉丁字母 "CS" 表示 2014 版往来港澳通行证
            self.type = a[0:2]

            # 第3~11位，证件号码，其中第1位用大写拉丁字母“C”表示；第2~9位为顺序号，用8位阿拉伯数字表示
            self.number = a[2:11]

            # 第12位，3~11位的校验位，机读码校验位算法详见 GA/T 704.14-2015 第5章
            self.check_number = a[11]

            # 第13位，空位"<"

            # 第14~19位，证件有效期截止年月日，YYMMDD
            self.expiration_date = a[13:19]

            # 第20位，141~19位的检验位
            self.check_expiration_date = a[19]

            # 第21位，空位"<"

            # 第22~27位，持证人出生年月日，YYMMDD
            self.date_of_birth = a[21:27]

            # 第28位，22~27位的检验位
            self.check_date_of_birth = a[27]

            # 第29位，空位"<"

            # 第30位，3~12位、14~20位、22~28位的总检验位
            self.check_composite = a[29]
            ...
            self.valid_check_digits = [MRZCheckDigit.compute(self.number) == self.check_number,
                                       ...
                                       MRZCheckDigit.compute(
                                           self.expiration_date) == self.check_expiration_date and MRZ._check_date(
                                           self.expiration_date),
                                       MRZCheckDigit.compute(a[2:12] + a[13:20] + a[21:28]) == self.check_composite]
            self.valid_line_lengths = [len_a == 30]
            self.valid_misc = [self.type in ('CS','CD')]
            self.valid_score = 10 * sum(self.valid_check_digits) + sum(self.valid_line_lengths) + sum(self.valid_misc) + 1
            self.valid_score = 100 * self.valid_score // (40 + 1 + 1 + 1)
            self.valid_number, self.valid_date_of_birth, self.valid_expiration_date, self.valid_composite = self.valid_check_digits
    ...
    def _gbk_conv(self, name):
        try:
            gbk_map = {'A': '0', 'B': '1', 'C': '2', 'D': '3', 'E': '4', 'F': '5', 'G': '6', 'H': '7', 'I': '8', 'J': '9',
                       'K': 'A', 'L': 'B', 'M': 'C', 'N': 'D', 'O': 'E', 'P': 'F'}

            length = len(name)
            if isinstance(name, str) and length % 4 == 0:
                outName = ''
                for i in name:
                    outName = outName + gbk_map[i]

                hexName = binascii.a2b_hex(outName)
                chineseName = hexName.decode('gbk')
                return chineseName
            else:
                return ''
        except Exception as e:
...
class MRZOCRCleaner(object):
    ...
    def __init__(self):
        ...
        CHNHK2000 = ['A' * 2 + 'a' * 3 + 'A' * 36 + 'a' * 3,
                     '*' * 9 + 'n' + 'A' * 3 + 'n' * 7 + 'A' + 'n' * 7 + '*' * 14 + 'n' * 2]
        CHNTW2000 = ['A' * 2 + 'a' * 3 + 'A' * 36 + 'a' * 3,
                     '*' * 9 + 'n' + 'A' * 3 + 'n' * 7 + 'A' + 'n' * 7 + '*' * 14 + 'n' * 2]
        TWCHN2000 = ['A' * 2 + 'a' * 3 + 'A' * 36 + 'a' * 3,
                     '*' * 9 + 'n' + 'A' * 3 + 'n' * 7 + 'A' + 'n' * 7 + '*' * 14 + 'n' * 2]
        CHNHK2014 = ['a' * 3 + '*' + 'n' * 8 + 'A' + 'n' * 7 + 'A' + 'n' * 7 + 'A' + 'n']
        CHNTW2014 = ['a' * 3 + '*' + 'n' * 8 + 'A' + 'n' * 7 + 'A' + 'n' * 7 + 'A' + 'n']
        HKID = ['a' + 'n' * 6 + '*' + 'n' + '*' ]
        MACID = ['n' * 7 + '*' + 'n' + '*' ]
        CHNID = ['n' * 17 + '*' ]
        CHNHK2014ID = ['a' + '*' + 'n' * 7 ]

        self.FORMAT = {'TD1': TD1, 'TD2': TD2, 'TD3': TD3, 'HKID': HKID, 'CHNID': CHNID,
                       'MRVA': MRV, 'MRVB': MRV, 'CHNHK2000': CHNHK2000, 'MACID': MACID,
                       'CHNTW2000': CHNTW2000, 'CHNHK2014': CHNHK2014,
                       'TWCHN2000': TWCHN2000, 'CHNTW2014': CHNTW2014,
                       'CHNHK2014ID': CHNHK2014ID}

        # Fixers
        a = {'0': 'O', '1': 'I', '2': 'Z', '4': 'A', '5': 'S', '6': 'G', '8': 'B'}
        n = {'B': '8', 'C': '0', 'D': '0', 'G': '6', 'I': '1', 'O': '0', 'Q': '0', 'S': '5', 'Z': '2', 'Y': '7'}

        self.FIXERS = {'a': a, 'A': a, 'n': n, 'N': n, '*': {}}

    def _split_lines(self, mrz_ocr_string):
        return [ln for ln in mrz_ocr_string.replace(' ', '').replace('く', '<').split('\n') if (len(ln) >= 20 or '<<' in ln)]

    def __call__(self, mrz_ocr_string, formatType=None):
        """
    ...
    def _fix_line(self, line, formatType, line_idx):
        ln = list(line)
        for j in range(len(ln)):
            ln[j] = self._fix_char(ln[j], formatType, line_idx, j)
        return ''.join(ln)

    def _fix_char(self, char, formatType, line_idx, char_idx):
        fmt = self.FORMAT[formatType][line_idx]
        if char_idx >= len(fmt):
            return char
        else:
...
if __name__ == "__main__":
    mrz = MRZ(['W<CHNMFLHOHOKMAKE<<<<<<<<<<<<<<<<<<<<<<<<DAC', 'W821232446CHN1010134M181015020204401<<<<<<54'])
    print(MRZOCRCleaner.apply('P<MYSL OW<YEE<HON<<<<<<<<<<<<<<<<<<<<<<<<<<<<'))
    print(mrz)
    print(mrz.nationality, mrz.country, mrz.number, mrz.date_of_birth, mrz.expiration_date, mrz.names, mrz.surname,
          mrz.sex, mrz.mrz_type, mrz.valid)
